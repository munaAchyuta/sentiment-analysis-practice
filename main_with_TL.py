...
tf.app.flags.DEFINE_string('dataset', 'data/ms_train_data.txt', 'training file')#St_train_cleaned_data_5k_new
tf.app.flags.DEFINE_string('devset', 'data/dev_data.txt', 'development file')#St_test_data , ms_St1k_train_data
tf.app.flags.DEFINE_string('testset', 'data/ms_test_data.txt', 'testing file')#ms_St500_test_data.txt, St_test_data_ex500.txt
...
class LSTM(object):
    ...
    def BiRNN(self, inputs, num_layers=1):
        ...
        lstm_fw_cell = rnn.BasicLSTMCell(self.n_hidden / 2, state_is_tuple=True, forget_bias=1.0, reuse=False)
        lstm_fw_cell = tf.contrib.rnn.DropoutWrapper(lstm_fw_cell, output_keep_prob=1. - self.keep_prob1)
        # Backward direction cell
        lstm_bw_cell = rnn.BasicLSTMCell(self.n_hidden / 2, state_is_tuple=True, forget_bias=1.0, reuse=False)
    ...
    def ConvLSTM_tl(self, inputs, num_layers=2, conv_len=5):
        ...
        outputs = self.ConvLSTM(inputs)
        output = tf.layers.dense(inputs=outputs, units=300, activation=tf.nn.relu)
        #hidden = tf.nn.dropout(hidden, keep_prob=self.keep_prob2)
        #output = tf.layers.dense(inputs=outputs, units=300, activation=tf.nn.relu)
    ...
    def TextCNN(self, inputs, filter_sizes=[3,5], num_filters=150):
        ...
        # apply attention
        #h_attn = self.apply_attention(h_pool_flat, batch_size, self.max_sentence_len, self.n_hidden)
    ...
    def TextCNN_tl(self, inputs, filter_sizes=[3,5], num_filters=150):
        ...
        outputs = self.TextCNN(inputs, filter_sizes=[3,5], num_filters=num_filters)
        hidden = tf.layers.dense(inputs=outputs, units=self.n_hidden, activation=tf.nn.relu)
        hidden = tf.nn.dropout(hidden, keep_prob=self.keep_prob2)
        output = tf.layers.dense(inputs=outputs, units=self.n_hidden)
    ...
    def CEA_tl(self, inputs, entity, hopnum=3):
        print('I am CEA_tl.')
        batch_size = tf.shape(inputs)[0]
        entity0 = tf.reshape(entity, [-1, 1, self.embedding_dim])
        entity1 = inputs * entity0
        ...
        #hidden = tf.layers.dense(inputs=outputs, units=self.n_hidden, activation=tf.nn.relu)
        #hidden = tf.nn.dropout(hidden, keep_prob=self.keep_prob2)
        h = tf.layers.dense(inputs=h, units=self.n_hidden, activation=tf.nn.relu)
        
        return LSTM.softmax_layer(h, self.weights_tl['softmax_tl'], self.biases_tl['softmax_tl'], self.keep_prob2)
    ...
    def run(self, PreEmbedding, word2id, p2id, p2v):
        ...
        if FLAGS.method == "CEA_tl":
            prob = self.CEA_tl(inputs, entity, hopnum = FLAGS.hopnum)
        if FLAGS.method == "SLSTM":
        ...
        with tf.name_scope('loss'):
            ...
            elif FLAGS.method in ["SLSTM","MLSTM","ConvLSTM"]:
            ...
            elif FLAGS.method in ["BiLSTM"]:
                reg_loss = tf.nn.l2_loss(self.weights['softmax']) + tf.nn.l2_loss(self.biases['softmax'])
                #reg_loss = tf.nn.l2_loss(self.bilstm_weights['bilstm_softmax_w']) + tf.nn.l2_loss(self.biases['softmax'])
                reg_loss = reg_loss * self.l2_reg
                cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=tf.log(prob), labels=tf.cast(self.y, tf.float32))) + reg_loss
        ...
        with tf.name_scope('train'):
            if FLAGS.method in ["ConvLSTM_tl","TextCNN_tl","CEA_tl"]:
        ...
        with tf.Session() as sess:
            ...
            if self.restore:
                ...
                if FLAGS.method in ["ConvLSTM_tl","TextCNN_tl","CEA_tl"]:
                    ...
                    savefilename_pre = 'ME-ABSA-AAAI-' + 'CEA'
                    #saver_pre.restore(sess, './models_convlstm_st/'+savefilename)
                    #saver_pre.restore(sess, './models_fasttext_convlstm_St/'+savefilename_pre)
                    #saver_pre.restore(sess, './models_fasttext_cnn_St/'+savefilename_pre)
                    #saver_pre.restore(sess, './models_cnn_St_4Lwords/'+savefilename_pre)
                    saver_pre.restore(sess, './models_cea_St/'+savefilename_pre)
                max_acc = 0.
